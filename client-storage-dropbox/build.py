...
APPEND_LINE = 'window["ClientStorage"] = ClientStorage;\n'
IIFE_CALL = '.call(this);\n'
...
def build():
    ...
    build_file = our_path / 'build' / 'clientStorage.min.js'
    ...
    for filename in glob(js_files):
        with open(filename, 'r+t', encoding='utf-8', newline='\n') as jsfile:
            ...
            if os.path.basename(filename) == 'clientStorage.js':
                contents = contents.replace('require("dropbox")', 'window.Dropbox')
